...
from test_model.entity import TFLiteModel
...
def thread_test(model_file, test_image, label, imgname, path, show_wrong=False, save_plabel=False, save_wrong_img=False):
    global top1_correct, top3_correct, CLASSES, total_time, thread_count
    tflite = TFLiteModel(str(model_file))

    start_time = time.time()
    output = tflite.output(test_image)
...
def test_tflite(model_file,
            ...
            for path, _, imgnames in os.walk(os.path.join(dirpath, dirname)):
                for imgname in imgnames:
                    ...
                    t = threading.Thread(target=thread_test, args=(
                        model_file, image, label, imgname, path, False, save_plabel, save_wrong_img))
                    t.start()
                    thread_count += 1
    ...
    while thread_count>0:
        time.sleep(0.01)
    print("Top1 Acc:", top1_correct / total, "Top3 Acc:", top3_correct / total, "Average time:", total_time / total)
